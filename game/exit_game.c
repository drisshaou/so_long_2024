...
/*   Updated: 2024/07/29 08:32:45 by drhaouha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	destroy_all_images(t_slng *so)
{
	if (so->playground->img->ptr != NULL)
		mlx_destroy_image(so->mlx, so->playground->img->ptr);
	if (so->playground_full->img->ptr != NULL)
		mlx_destroy_image(so->mlx, so->playground_full->img->ptr);
	if (so->playground_tmp->img->ptr != NULL)
		mlx_destroy_image(so->mlx, so->playground_tmp->img->ptr);
