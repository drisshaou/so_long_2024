...
/*   Updated: 2024/07/31 07:35:21 by drhaouha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_img	*playground_layer;
	t_pt	p;

	p = coord_to_position(so,
			(t_coord){so->player->col, so->player->row}, *so->map_dim);
	playground_layer = so->playground_full->img;
	if (p.x == so->player->img->x && p.y == so->player->img->y)
		playground_layer = so->playground->img;
	return (playground_layer);
}
...
void	clean_frame(t_slng *so)
{
	...
	layer = get_background_layer(so, get_playground_layer(so),
