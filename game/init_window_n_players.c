...
/*   Updated: 2024/07/29 11:48:04 by drhaouha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_ghost(t_slng *so)
{
	...
	so->ghost->id = 'G';
	pos = get_pos_in_playground(so, *so->map_dim, so->ghost->id);
...
void	init_player(t_slng *so)
{
	...
	so->player->id = 'P';
	pos = get_pos_in_playground(so, *so->map_dim, so->player->id);
...
void	init_window_n_players(t_slng *so)
{
	...
	so->playground->img->y = 0;
	if (so->playground->img->width <= so->width)
		so->width = so->playground->img->width;
	if (so->playground->img->height <= so->height)
		so->height = so->playground->img->height;
	init_player(so);
	init_ghost(so);
	so->finished = false;
	so->has_ghost = true;
