...
/*   Updated: 2024/07/29 08:49:31 by drhaouha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	game_is_over(t_slng *so)
{
	...
	{
		__timer(60000000);
		apply_grayscale(so->playground_tmp->img);
		insert_image_to(so->playground_tmp->img, so->game_over,
			(t_pt){so->game_over->x - so->playground_tmp->img->x,
			so->game_over->y - so->playground_tmp->img->y});
		draw_frame(so);
	}
	exit_game(so);
}
